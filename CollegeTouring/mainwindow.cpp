...
//        }
    ...
    if(index == 4){
        qWarning() << "Beginning sort on: ";
        collegeMap.BFS(0);
        ui->BFSwidget->clear();
        for(int i = 0; i < collegeMap.ans.size(); i++)
        {
            ui->BFSwidget->addItem(collegeMap.ans.at(i).origin/* + " -> " + collegeMap.ans.at(i).dest + " Dist: " + QString::number(collegeMap.ans.at(i).dist)*/);
            qWarning() << collegeMap.ans.at(i).origin << " -> " << collegeMap.ans.at(i).dest;
        }
        ui->BFSwidget->setSelectionMode(QAbstractItemView::SingleSelection);
        ui->pushButton_2->setEnabled(false);
       // selected
    }
    else
